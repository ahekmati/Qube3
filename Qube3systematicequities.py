...
def main():
    ...
    for symbol in symbols:
        ...
        if df_daily is not None and "Daily" in best:
            ...
            print(f"[Scan] {symbol}: Found {len(dsigs)} daily signals (SMMA {p['fast']}/{p['slow']}).")
            for s in dsigs:
            ...
            print(f"[Scan] {symbol}: No/missing Daily combo.")
        # --- 8-HOUR
        ...
        if df_8h is not None and "8-Hour" in best:
            ...
            print(f"[Scan] {symbol}: Found {len(esigs)} 8H signals (SMMA {p['fast']}/{p['slow']}).")
            for s in esigs:
            ...
            print(f"[Scan] {symbol}: No/missing 8-Hour combo.")
        # --- 4-HOUR
        ...
        if df_4h is not None and "4-Hour" in best:
            ...
            print(f"[Scan] {symbol}: Found {len(fsigs)} 4H signals (SMMA {p['fast']}/{p['slow']}).")
            for s in fsigs:
