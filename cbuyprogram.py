...
MAX_ALLOC = 0.15
MAX_POSITIONS = 5
DEF_SWING_WINDOW = 10      # bars to look back for swing low
DEFENSE_STOP_PCT = 0.02    # set defensive stop at max(swing low, 2% below last close)
...
def fetch_ohlcv(ib, symbol, ndays, tf='1 day'):
...
def apply_index_defense(ib, state, spy_bearish):
    ...
    for symbol, pos in state.data["positions"].items():
        if symbol not in ib_pos or symbol in EXCEPTION_TICKERS:
            continue
        try:
            df = fetch_ohlcv(ib, symbol, DEF_SWING_WINDOW + 2, '1 day')
...
def update_trailing_stops(ib, state):
    ...
    for symbol, pos in state.data["positions"].items():
        ...
        try:
            df = fetch_ohlcv(ib, symbol, 30, '1 day')
            shares = pos['shares']
            contract = Stock(symbol, 'SMART', 'USD')
            for trade in ib.trades():
                if (trade.contract.symbol == symbol 
...
def main():
    ...
    spy_df = fetch_ohlcv(ib, 'SPY', 120, '1 day')
    ...
    qualified = []
    ib_pos_set = {p.contract.symbol for p in ib.positions()}
    for symbol in symbols:
        if symbol in state.data['positions'] or symbol in ib_pos_set:
        ...
        df_daily = fetch_ohlcv(ib, symbol, 120, '1 day')
            ...
            for s in dsigs:
                ...
                if score is not None:
                    qualified.append({
                        ...
                        "timeframe":s["label"], "signal_date":str(s["date"]), "df":df_daily
                    })
        else:
            print(f"[Scan] {symbol}: No daily bar data, skipping.")
        print(f"[Scan] Scanning {symbol} (4H bars)...")
        df_4h = fetch_ohlcv(ib, symbol, 120, '4 hours')
            ...
            for s in fsigs:
                ...
                if score is not None:
                    qualified.append({
                        "symbol":symbol, "score":score, "rvol":rvol,
                        "momentum":momentum, "price":s["close"], "signal_type":s["signal_type"],
                        "timeframe":s["label"], "signal_date":str(s["date"]), "df":df_4h
                    })
        else:
    ...
    print("Idx | Symbol  | Timeframe | Type    | Date                | Price    | Score   | RVOL    | MOM")
    print("----|---------|-----------|---------|---------------------|----------|---------|---------|---------")
    for i,q in enumerate(top_qualified):
        print(f"{i+1:>3} | {q['symbol']:<7} | {q['timeframe']:<9} | {q['signal_type']:<7} | {q['signal_date']:<19} | {q['price']:<8.2f} | {q['score']:<6.3f} | {q['rvol']:<6.2f} | {q['momentum']:<6.3f}")
    ...
    for q in top_qualified:
        ...
        # ----------- SHOW BOTH SIGNAL AND LIVE PRICE -------------
        live_px = get_underlying_price(ib, Stock(symbol, 'SMART', 'USD'))
        print(f"[Signal Price Compare] Signal bar close: ${q['price']:.2f} | Current live price: ${live_px:.2f}")
        # ---------------------------------------------------------
    ...
    # Defensive portfolio-wide stop raise if SPY regime turns bearish
