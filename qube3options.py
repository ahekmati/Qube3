...
best_smma_combos = {
    "SSO":   {"1h": (19, 58), "2h": (15, 18), "4h": (5, 18)},
    "SPXS":  {"1h": (19, 53), "2h": (13, 43), "4h": (5, 23)},
    "TQQQ":  {"1h": (5, 18),  "2h": (17, 58), "4h": (15, 23)},
    "SQQQ":  {"1h": (7, 18),  "2h": (11, 48), "4h": (13, 58)},
    "SVXY":  {"1h": (13, 58), "2h": (11, 58), "4h": (19, 53)},
    "SOXL":  {"1h": (5, 23),  "2h": (13, 43), "4h": (17, 53)},
    "IBIT":  {"1h": (11, 18), "2h": (7, 38),  "4h": (19, 53)},
    "TSLL":  {"1h": (15, 18), "2h": (17, 18), "4h": (19, 23)},
    "SPY":   {"1h": (19, 58), "2h": (17, 18), "4h": (5, 18)},
    "QQQ":   {"1h": (5, 43),  "2h": (19, 53), "4h": (7, 23)},
}
user_tickers = list(best_smma_combos)
tf_map = {"1h": "1 hour", "2h": "2 hours", "4h": "4 hours"}

def recent_avg_swing_high(df, lookback=14):
    ...
    for i in range(1, len(highs)-1):
        if highs.iloc[i] > highs.iloc[i-1] and highs.iloc[i] > highs.iloc[i+1]:
            swings.append(highs.iloc[i])
    if swings:
        return float(np.mean(swings))
    elif len(highs) > 0:
        return float(np.mean(highs))
    else:
        return None
...
def recent_smma_signals(df, fast, slow, days_back=14, label="DAILY"):
    if df is None or len(df) < slow + days_back:
        return []
    ...
    for i in range(len(df) - lookback, len(df)):
        ...
        reentry = (
            ...
            and df['close'].iloc[prev] < df['smma_slow'].iloc[prev]
            and df['close'].iloc[curr] > df['smma_slow'].iloc[curr]
        )
        if bull or reentry:
            signals.append({
...
def apply_index_defense(ib, state, spy_bearish):
    ...
    for symbol, pos in state.data["positions"].items():
        ...
        try:
            ...
            if df is None or len(df) < DEF_SWING_WINDOW + 2:
                continue
...
def main():
    ...
    for symbol in symbols:
        ...
        if not spy_long_allowed and symbol not in EXCEPTION_TICKERS:
            ...
            continue
        combos = best_smma_combos[symbol]
        for tf, tf_name in tf_map.items():
            fast, slow = combos[tf]
            df = cached_fetch_ohlcv(ib, symbol, 120 if tf == "1h" else 240 if tf == "2h" else 360, tf_name)
            if df is not None:
                signals = recent_smma_signals(df, fast, slow, 7 if tf == "1h" else 14, label=tf.upper())
                swing_high_avg = recent_avg_swing_high(df, 14)
                for s in signals:
                    ...
                    if score is not None:
                        ...
                        qualified.append({
                            ...
                            "age": age,
                            "df": df,
                            "ma_combo": f"SMMA {fast}/{slow}",
                            "avg_swing_high": swing_high_avg
                        })
            else:
                print(f"[Scan] {symbol}: No data for {tf.upper()} timeframe.")
    ...
    print("Idx | Symbol  | TF | MA Used     | Type    | SignalAge | Price    | RVOL    | MOM    | AvgSwing High")
    print("----|---------|----|------------|---------|-----------|----------|---------|--------|--------------")
    for i, q in enumerate(top_unique):
        swing_high_str = f"{q['avg_swing_high']:.2f}" if q['avg_swing_high'] is not None else "N/A"
        print(f"{i + 1:>3} | {q['symbol']:<7} | {q['timeframe']:<3} | {q['ma_combo']:<11} | {q['signal_type']:<7} | "
              f"{q['age']:<9} | {q['price']:<8.2f} | {q['rvol']:<6.2f} | {q['momentum']:<6.3f} | {swing_high_str}")
    for q in top_unique:
        symbol = q['symbol']
        if symbol in state.data['positions'] or symbol in {p.contract.symbol for p in ib.positions()}:
