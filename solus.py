...
def maybe_load_increments():
    ...
    try:
        ...
        if qi: QUOTE_TICK = Decimal(str(qi))
        if bi: BASE_STEP = Decimal(str(bi))
    except Exception:
        pass

def get_accounts_list():
    accts = client.get_accounts()
    print("\n[DEBUG] Raw accounts_list():\n", json.dumps(accts if isinstance(accts, dict) else accts.__dict__, indent=2, default=str))
    ...
    print(f"[DEBUG] Parsed accounts_list length: {len(accounts_list) if accounts_list else 0}")
...
def get_open_base_size(product_id):
    ...
    for a in get_accounts_list():
        ...
        total = a.get("available_balance") if isinstance(a, dict) else getattr(a, "available_balance", None)
        val = total.get("value") if isinstance(total, dict) else (total if isinstance(total, (int, float, str)) else 0.0)
        print(f"[DEBUG] Account {curr}: available_balance = {val}")
        if curr == base:
            try:
                parsed = float(val)
                print(f"[DEBUG] Matched base {base}, parsed available_base_size = {parsed}")
                return parsed
            except Exception:
                print(f"[DEBUG] Failed to parse base available value: {val}")
                return 0.0
    print(f"[DEBUG] Base {base} not found, returning 0.0")
    return 0.0
    ...
    for a in get_accounts_list():
        ...
        if curr == base:
            try:
                ...
                print(f"[DEBUG] Matched base {base}, parsed available_base_precise = {result}")
                return result
            except Exception:
                print(f"[DEBUG] Failed to Decimal-parse base available value: {val}")
                return Decimal("0")
    print(f"[DEBUG] Base {base} not found, returning Decimal('0')")
    return Decimal("0")
...
def get_quote_available(product_id):
    ...
    for a in get_accounts_list():
        ...
        if curr == quote:
            try:
                parsed = float(val)
                print(f"[DEBUG] Matched quote {quote}, parsed available_quote = {parsed}")
                return parsed
            except Exception:
                print(f"[DEBUG] Failed to parse quote available value: {val}")
                return 0.0
    print(f"[DEBUG] Quote {quote} not found, returning 0.0")
    return 0.0
...
def print_balances(product_id):
    ...
    print(f"[BAL] {base} available: {base_avail}")
...
def fetch_ohlcv(product_id, granularity, limit):
    ...
    rows = None
    if isinstance(resp, dict) and "candles" in resp: rows = resp["candles"]
    if rows is None and hasattr(resp, "candles"): rows = getattr(resp, "candles")
    if rows is None and isinstance(resp, list): rows = resp
    if rows is None and hasattr(resp, "to_dict"): d = resp.to_dict(); rows = d.get("candles")
    if rows is None and hasattr(resp, "dict"): d = resp.dict(); rows = d.get("candles")
    if rows is None or not isinstance(rows, list) or len(rows) == 0: raise RuntimeError("No candles returned")
    clean = []
    for r in rows:
        clean.append(dict(zip(['start','low','high','open','close','volume'], r)))
...
# ========= UTILS =========
def client_id(prefix):
    return f"{prefix}_{uuid.uuid4().hex[:12]}"  # Unique client order ids. [web:62]

def align_to_day_epoch(ts_utc: datetime) -> int:
    d = ts_utc.date()  # Align to 00:00:00 UTC. [web:62]
    aligned = datetime(d.year, d.month, d.day, tzinfo=UTC)  # TZ-aware. [web:62]
    return int(aligned.timestamp())  # UNIX seconds. [web:62]

def last_completed_day_end() -> int:
    now_utc = datetime.now(UTC) - timedelta(minutes=1)  # Avoid partial candle. [web:62]
    return align_to_day_epoch(now_utc)  # Last completed day boundary. [web:62]

def fmt_price(x: Decimal) -> str:
    q = QUOTE_TICK.normalize()
    decimals = -q.as_tuple().exponent if q.as_tuple().exponent < 0 else 0
    return f"{x:.{decimals}f}"  # [web:63]
...
def round_price_to_tick(x: Decimal) -> Decimal:
    return (x / QUOTE_TICK).to_integral_value() * QUOTE_TICK  # [web:63]

def floor_size_to_step(x: Decimal) -> Decimal:
    steps = (x / BASE_STEP).to_integral_value(rounding=ROUND_FLOOR)
    return steps * BASE_STEP  # [web:63]

def maybe_load_increments():
    global QUOTE_TICK, BASE_STEP
    try:
        p = client.get_product(product_id=PRODUCT_ID)  # product metadata [web:63]
        qi = p.get("quote_increment") if isinstance(p, dict) else getattr(p, "quote_increment", None)
        bi = p.get("base_increment") if isinstance(p, dict) else getattr(p, "base_increment", None)
        if qi:
        ...
        pass  # keep defaults if unavailable [web:63]
...
def get_open_base_size(product_id):
    ...
    for a in get_accounts_list():
        ...
        if curr == base:
            ...
            val = avail.get("value") if isinstance(avail, dict) else (avail if isinstance(avail, (int, float, str)) else 0.0)
            try:
                return float(val)
            except Exception:
                return 0.0
...
def print_balances(product_id):
    ...
    base_avail = get_open_base_size(product_id)
    quote_avail = get_quote_available(product_id)
    print(f"[BAL] {base} available: {base_avail}")  # [web:62]
    print(f"[BAL] {quote} available: {quote_avail}")  # [web:62]
    others = []
    for a in get_accounts_list():
        curr = a.get("currency") if isinstance(a, dict) else getattr(a, "currency", None)
...
# (Keep prior logic -- order management, entry, TP/SL backfill...)

# Place the rest of your script below this line unchanged (entry, reentry, has_open_protective_order, etc.),
# using get_open_base_size and get_available_base_precise anywhere you need SOL position size or safe sizing,
# and relying on the debug logs above.

# ... Place the rest of your previous script logic verbatim, including order entry, TP/SL routines, etc. ...
# (To avoid a message cutoff, only the enhanced I/O and debug logic is shown above;
#  append the rest of the logic blocks here from your prior script!)
